...
class Artifact
  ...
  def valid?
    ...
    valid = (from_file && from_zip && from_file == from_zip || false)
    $log.info('artifact') { "#{to_s} is #{!valid && 'not ' || ''}valid" }
    valid
  end
  ...
  def md5sum_from_zip
    if s3_bucket.exists?(key)
      Tempfile.open('artifact') do |f|
        s3_bucket.download_key_to_file(key, f)
