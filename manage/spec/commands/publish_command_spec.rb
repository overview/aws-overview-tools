require 'commands/publish_command'

RSpec.describe Commands::PublishCommand do
  before(:each) {
    @runner = double()
  ...
  it 'should invoke PipelineCommandRunner.build' do
    expect(@pipelineCommandRunner).to receive(:publish).with('source', 'version', 'production')
